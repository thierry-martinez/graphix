...
class NoiseModelTester(DepolarisingNoiseModel):
    ...
    def __init__(
        ...
        super().__init__(prepare_error_prob=prepare_error_prob, x_error_prob=x_error_prob, z_error_prob=z_error_prob, entanglement_error_prob=entanglement_error_prob, measure_channel_prob=measure_channel_prob, measure_error_prob=measure_error_prob, rng=np.random.default_rng())
...
class TestNoisyRustDensityMatrixBackend(TestNoisyDensityMatrixBackend):
    def test_noiseless_noisy_hadamard(self, fx_rng: Generator) -> None:
        hadamardpattern = self.hpat()
        noiselessres = hadamardpattern.simulate_pattern(backend="densitymatrix", impl=RustDensityMatrix, rng=fx_rng)
        # noiseless noise model
        noisynoiselessres = hadamardpattern.simulate_pattern(
            backend="densitymatrix",
            impl=RustDensityMatrix,
            noise_model=NoiselessNoiseModel(),
            rng=fx_rng,
        )
        assert np.allclose(noiselessres, np.array([[1.0, 0.0], [0.0, 0.0]]))
        # result should be |0>
        assert np.allclose(noisynoiselessres, np.array([[1.0, 0.0], [0.0, 0.0]]))
    ...
    def test_noisy_measure_confuse_hadamard(self, fx_rng: Generator) -> None:
        ...
        # result should be |1>
        assert np.allclose(res, np.array([[0.0, 0.0], [0.0, 1.0]]))
        ...
        # result should be |1>
        assert np.allclose(res, np.array([[1.0, 0.0], [0.0, 0.0]])) or np.allclose(
            res,
            np.array([[0.0, 0.0], [0.0, 1.0]]),
        )
    ...
    def test_noiseless_noisy_rz(self, fx_rng: Generator) -> None:
        alpha = fx_rng.random()
        rzpattern = self.rzpat(alpha)
        noiselessres = rzpattern.simulate_pattern(backend="densitymatrix", impl=RustDensityMatrix)
        # noiseless noise model or NoiseModelTester() since all probas are 0
        noisynoiselessres = rzpattern.simulate_pattern(
            backend="densitymatrix",
            impl=RustDensityMatrix,
    ...
    def test_noisy_xz_rz(self, fx_rng: Generator) -> None:
        ...
        # 4 cases : no corr, noisy X, noisy Z, noisy XZ.
        assert (
            np.allclose(res, 0.5 * np.array([[1.0, np.exp(-1j * alpha)], [np.exp(1j * alpha), 1.0]]))
            or np.allclose(
                res,
                0.5
                * np.array(
    ...
    def test_noisy_measure_confuse_rz(self, fx_rng: Generator) -> None:
        ...
        assert (
            np.allclose(res, Ops.X @ exact @ Ops.X)
            or np.allclose(res, Ops.Z @ exact @ Ops.Z)
            or np.allclose(res, Ops.Z @ Ops.X @ exact @ Ops.X @ Ops.Z)
        )
        ...
        assert (
            ...
            or np.allclose(res, Ops.X @ exact @ Ops.X)
            or np.allclose(res, Ops.Z @ exact @ Ops.Z)
            or np.allclose(res, Ops.Z @ Ops.X @ exact @ Ops.X @ Ops.Z)
        )
