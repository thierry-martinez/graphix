...
class Pattern(InplaceParameterizable):
    ...
    def node_mapping(
        ...
        *,
        start: int = 0,
        avoids: Collection[Node] | None = None,
        preserves: Collection[Node] | None = None,
    ) -> dict[Node, Node]:
        """Compute an injective mapping of node indices.

        The resulting mapping can be passed to :meth:`reindex`.
        Fresh indices are allocated in increasing order, starting at
        ``start`` and skipping any values already used.

        Parameters
        ----------
        mapping: Mapping[Node, Node] | None, optional
            An initial (partial) mapping that will be extended.
            A ``ValueError`` is raised if the supplied mapping is not
            injective.
        start: int, optional
            The starting index from which fresh indices are allocated.
        avoids: Collection[Node] | None, optional
            Indices that must not be assigned to a node.
        preserves: Collection[Node] | None, optional
            Nodes that should not appear in the mapping.
            Their index will not be changed by :meth:`reindex`.

        Returns
        -------
        dict[Node, Node]
            A dictionary that maps each node of ``self`` (except those
            listed in ``preserves``) to a distinct integer index.

        Notes
        -----
        The resulting dictionary is guaranteed to be an extension of
        ``mapping``, i.e., all entries from ``mapping`` appear in the
        resulting dictionary. In particular, the constraints specified
        in ``avoids`` and ``preserves`` only affect nodes that are
        freshly generated by the function; they do not apply to the
        nodes already present in ``mapping``.
        """
        nodes = self.nodes()
        if mapping is None:
            result = {}
        ...
        if preserves:
            ...
            preserves_set = preserves if isinstance(preserves, AbstractSet) else set(preserves)
        else:
            preserves_set = set()
        candidate = start
        for node in sorted(nodes):
            if node in result or node in preserves_set:
                continue
    ...
    def compose(
        ...
        """
        nodes_p1 = self.nodes()  # Results contain preprocessed Pauli nodes
        nodes_p2 = other.nodes()
    ...
    def to_ascii(
        ...
        >>> pattern.to_ascii()
        'X(1,{0}) M(0,0) E(0,1) N(1)'
        >>> pattern.to_ascii(left_to_right=True)
        'N(1) E(0,1) M(0,0) X(1,{0})'
        ...
        >>> circuit.ccx(0, 1, 2)
        >>> pattern = circuit.transpile().pattern
        >>> pattern.to_ascii(limit=5)
        'X(24,{21}) Z(24,{20}) X(32,{31}) Z(32,{28})...(98 more commands)'
        >>> pattern.to_ascii(limit=5, left_to_right=True)
        'N(3) E(2,3) M(2,0) N(4)...(98 more commands)'
        >>> pattern.to_ascii(limit=None)
        'X(24,{21}) Z(24,{20}) X(32,{31}) Z(32,{28}) X(30,...E(4,1) [M(3,0)]{2} E(3,4) N(4) M(2,0) E(2,3) N(3)'
        """
    ...
    def signals(self) -> dict[int, set[int]]:
    ...
    def to_pauliflow(self) -> PauliFlow[Measurement]:
        ...
        ------
        FlowError
            If the pattern is empty or if no Pauli flow is compatible with the pattern corrections.
        ValueError
            If `N` commands in the pattern do not represent a :math:`|+\rangle` state or if the
            pattern corrections form closed loops.
    ...
    def _measurement_order_depth(self) -> list[int]:
        ...
        """
        partial_order_layers = self.partial_order_layers()
    ...
    def sort_measurement_commands(self, meas_order: list[int]) -> list[command.M]:
        ...
        """
        meas_dict = self.measurement_commands()
        return [meas_dict[i] for i in meas_order]

    def measurement_commands(self) -> dict[int, command.M]:
